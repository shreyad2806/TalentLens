"""
Test script for the PineconeAdapter.

This script tests the PineconeAdapter implementation to verify that it
...
def test_pinecone_adapter_with_credentials():
    ...
    if not api_key or not index_name:
        ...
        print("  export PINECONE_NAMESPACE=your_namespace (optional)")
        print()
        return True
    ...
    print(f"  Namespace: {os.environ.get('PINECONE_NAMESPACE', 'default')}")
    print()
    ...
    if health.get('healthy', False):
        ...
        try:
            record = service.fetch("test-pinecone-adapter-001")
            if record:
        ...
        try:
            result = service.delete(["test-pinecone-adapter-001"])
            if result['success']:
...
def test_pinecone_adapter():
    """
    Test the PineconeAdapter implementation.
    
    This test verifies that the PineconeAdapter correctly implements
    the VectorStore interface and handles errors gracefully.
    """
    print_header("PINECONE ADAPTER TEST")
    print()
    ...
    # Final result
    print_header("PINECONE ADAPTER TEST PASSED")
    print_success("All tests passed")
    print()
    print("🚀 PineconeAdapter Ready")
    print()
    print("Note: Full integration test requires Pinecone credentials.")
    print("Set PINECONE_API_KEY and PINECONE_INDEX_NAME environment variables to test with Pinecone.")
    return True


if __name__ == "__main__":
    try:
        success = test_pinecone_adapter()
